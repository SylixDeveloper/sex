...
use function Amp\File\unlink as AmpUnlink;
...
*/
                ...
                if (preg_match('/^[\#\!\/\.]?(عاح|اوف|ماچ|بوس|اوه)$/i', $txt)) {
                    if (isset($reply_to_msg_id)) {
                        if ($type == 'channel' || $type == 'supergroup') {
                            $replyID = yield $this->channels->getMessages(['channel' => $chat_id, 'id' => [$reply_to_msg_id]]);
                        } else {
                        ...
                        $media = $replyID['messages'][0]['media'];
                        $type = ($media['_'] == 'messageMediaDocument') ? 'video' : 'photo';
                        $filename = ($type == 'video') ? 'data/dl.mp4' : 'data/a.jpg';
                        if (yield exists(__DIR__.'/'.$filename)){
                            yield AmpUnlink(__DIR__.'/'.$filename);
                        }
                        yield $this->downloadToFile($media, $filename);
